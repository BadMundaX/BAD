...
class Call(PyTgCalls):
    ...
    async def _queue_autoplay_track(self, chat_id: int, last_track: dict, _):
        ...
        related = None
        try:
            related = await Platform.YouTube.related_video(videoid, settings_chat_id)
        ...
        if not next_id or next_id == videoid:
            ...
            for query in [
                ...
                try:
                    fallback, fallback_id = await Platform.YouTube.track(query)
                except Exception as e:
        ...
        try:
            title, duration_min, duration_sec, thumbnail, next_vidid = await Platform.YouTube.details(
